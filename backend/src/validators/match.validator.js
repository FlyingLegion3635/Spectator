...
const createMatchEntrySchema = z
  .object({
    ...
    fireRate: z.coerce.number().min(0).optional().default(0),
    shotsAttempted: z.coerce.number().int().min(0).optional(),

    // Compatibility alias for existing Flutter naming.
    ballsShot: z.coerce.number().int().min(0).optional(),

    accuracy: z.coerce.number().min(0).max(1),
    calculatedPoints: z.coerce.number().int().min(0),
